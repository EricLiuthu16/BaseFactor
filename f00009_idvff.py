...
class IDVFF(BaseFactor):
    ...
    def __init__(
            self,
            factor_name='f00009',
            factor_parameters={'lagTradeDays': 250, 'factor_input': 'f00001'}
    ):

        # Initialize super class.
        super(IDVFF, self).__init__(factor_name=factor_name, factor_parameters=factor_parameters)
...
if __name__ == '__main__':
    ...
    # 测试
    idfvv.prepare_data(sdt, edt)
    test = ridvff.test_calculation(dt = '2021-10-12')

    # 计算并储存数据
    idvff.generate_factor_all(sdt, edt, process=5, nan_policy='keep')
    idvff.save()
